...
import type { ReceivablesAgingItem } from '@/lib/reports';
...
export default function ReceivablesAgingPage() {
  ...
  const handleRowClick = (record: ReceivablesAgingItem) => {
  ...
  const columns: ColumnsType<ReceivablesAgingItem> = [
    {
      title: 'Customer',
  ...
  return (
    <div>
      ...
      <Space direction="vertical" size="small" style={{ marginBottom: 16 }}>
        <Text type="secondary">Outstanding customer invoices grouped by aging period</Text>
        <Space>
      ...
      {isLoading ? (
        ...
        <>
          ...
          <Card>
            {data?.data.length === 0 ? (
              <Empty description="No outstanding receivables" />
            ) : (
              <Table
                columns={columns}
